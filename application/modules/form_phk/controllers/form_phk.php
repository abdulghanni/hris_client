...
class Form_phk extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->database();
        $this->load->model('form_phk/form_phk_model','main');
    ...
    function input()
    {
        $this->data['title'] = "Input - Form phk";
    ...
    {
        $this->data['title'] = "Detail - Form phk";
        $sess_id = $this->session->userdata('user_id');
        $nik = get_nik($sess_id);
        ...
        if (!$this->ion_auth->logged_in())
        {
            ...
            //redirect them to the login page
            redirect('auth/login', 'refresh');
        }elseif(!is_user_app_lv1($nik,$id,'users_phk')&&!is_user_app_lv2($nik,$id,'users_phk')&&!is_user_app_lv3($nik,$id,'users_phk')&&!is_user_app_lv4($nik,$id,'users_phk')&&!is_user_app_lv5($nik,$id,'users_phk')&&!is_admin()&&!is_hrd_cabang($bu)&&!is_hrd_pusat($nik,15)&&!is_user_logged($nik,$id,'users_phk')&&!is_creator($nik,$id,'users_phk')&&!is_cc_notif($nik,$bu,15)){
            return show_error('Anda tidak dapat mengakses halaman ini.');
        }else{
           $this->data['id'] = $id;
            $sess_id = $this->data['sess_id'] = $this->session->userdata('user_id');
            $sess_nik = $this->data['sess_nik'] = get_nik($sess_id);
            $user_id = getValue('user_id', 'users_phk', array('id'=>'where/'.$id));
            $this->data['user_nik'] = get_nik($user_id);
            $this->data['row'] = $this->main->detail($id)->row();
            $this->data['_num_rows'] = $this->main->detail($id)->num_rows();
            $this->data['user_id'] =$user_id = getValue('created_by', 'users_phk', array('id'=>'where/'.$id));
            $first_name = getValue('first_name', 'users', array('id'=>'where/'.$user_id));
            $this->data['user_folder'] = $user_id.trim($first_name).'/phk/';
        ...
        if (!$this->ion_auth->logged_in())
        {
            ...
            else
            {
                ...
                $additional_data = array(
                    'date_phk'           => date('Y-m-d',strtotime($this->input->post('date_phk'))),
                ...
                if ($this->form_validation->run() == true && $this->main->create_($user_id, $additional_data))
                {
                     ...
                     if(!empty($user_app_lv1)){
                        ...
                        $this->approval->request('hrd', 'phk', $phk_id, $user_id, $this->detail_email($phk_id));
                        if(!empty(getEmail($this->approval->approver('phk'))))$this->send_email(getEmail($this->approval->approver('phk')), $subject_email, $isi_email);
                     }

                     redirect('form_phk', 'refresh');
                    //echo json_encode(array('st' =>1, 'phk_url' => $phk_url));    
                }
            }
        }
    }
    ...
    function upload_attachment($id)
    {
        ...
        if(!is_dir("./uploads/$user_folder/phk/")){
        mkdir("./uploads/$user_folder/phk/", 0777);
        }


        $path = "./uploads/$user_folder/phk/";
    ...
    function send_notif($id, $type){
        ...
        if($is_app==0){
            ...
            if(!empty(getEmail($user_phk_id)))$this->send_email(getEmail($user_phk_id), $subject_email, $isi_email);
        }else{
            $this->approval->update_approve('phk', $id, $approval_status, $this->detail_email($id));
            if(!empty(getEmail($user_phk_id)))$this->send_email(getEmail($user_phk_id), get_form_no($id).'['.$approval_status_mail.']Perubahan Status Pengajuan Permohonan phk dari Atasan', $isi_email);
        }
        if($type !== 'hrd' && $approval_status == 1)
        {
            $lv = substr($type, -1)+1;
            $lv_app = 'lv'.$lv;
            $user_app = ($lv<6) ? getValue('user_app_'.$lv_app, 'users_phk', array('id'=>'where/'.$id)):0;
            if(!empty($user_app)){
                $this->approval->request($lv_app, 'phk', $id, $user_phk_id, $this->detail_email($id));
                if(!empty(getEmail($user_app)))$this->send_email(getEmail($user_app), $subject_email_request, $isi_email_request);
            }else{
                $this->approval->request('hrd', 'phk', $id, $user_phk_id, $this->detail_email($id));
                if(!empty(getEmail($this->approval->approver('phk', $user_id))))$this->send_email(getEmail($this->approval->approver('phk', $user_id)), $subject_email_request, $isi_email_request);
            }
    ...
    function form_phk_pdf($id)
    {
        ...
        $user_id = getValue('user_id', 'users_phk', array('id'=>'where/'.$id));
        $this->data['user_nik'] = get_nik($user_id);
        $form_phk = $this->data['form_phk'] = $this->main->detail($id)->result();
        $this->data['_num_rows'] = $this->main->detail($id)->num_rows();

        $creator = getValue('created_by', 'users_phk', array('id'=>'where/'.$id));
        $creator = get_nik($creator);
        $this->data['form_id'] = 'PHK';
        $this->data['bu'] = get_user_buid($creator);
        $loc_id = get_user_locationid($creator);
        $this->data['location'] = get_user_location($loc_id);
        $date = getValue('created_on','users_phk', array('id'=>'where/'.$id));
        $this->data['m'] = date('m', strtotime($date));
        $this->data['y'] = date('Y', strtotime($date));
        $this->load->library('mpdf60/mpdf');
        $html = $this->load->view('phk_pdf', $this->data, true); 
        $this->mpdf = new mPDF();
    ...
    function pdf_blank($id)
    {
        ...
        $this->mpdf->AddPage('P', // L - landscape, P - portrait
            ...
            10); // margin footer
        $this->mpdf->WriteHTML($html);
        $this->mpdf->Output('form_template-phk.pdf', 'I');
        }
    }
    ...
    function _render_page($view, $data=null, $render=false)
    {
        ...
        if ( ! $render)
        {
            $this->load->library('template');
                ...
                if(in_array($view, array('form_phk/index')))
                {
                ...
                elseif(in_array($view, array('form_phk/input')))
                {
                    ...
                    $this->template->add_js('form_phk.js');
