...
class Form_phk_model extends CI_Model {

    var $table = 'users_phk';
    var $join1  = 'users';
    var $column = array('users_phk.id', 'nik_karyawan','karyawan', 'nik_pengaju', 'pengaju', 'date_phk', 'created_on'); //set column field database for order and search
    ...
    private function _get_datatables_query($f)
    {
        ...
        $this->db->select(array(
                'users_phk'.'.id as id',
                'users_phk'.'.app_status_id_lv1',
                'users_phk'.'.app_status_id_lv2',
                'users_phk'.'.app_status_id_lv3',
                'users_phk'.'.app_status_id_hrd',
                'users_phk'.'.user_app_lv1',
                'users_phk'.'.user_app_lv2',
                'users_phk'.'.user_app_lv3',
                'users_phk'.'.created_by',
            ...
            $this->db->from('users_phk');


            $this->db->join($this->join1, 'users_phk.user_id = users.id', 'left');
            $this->db->join($this->join1.' as pengaju', 'users_phk.created_by = pengaju.id', 'left');
     ...
     public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        //filter out any data passed that doesnt have a matching column in the form phk table
        //and merge the set group data and the additional data

        if (!empty($additional_data)) $data = array_merge($additional_data, $data);


        // insert the new form_phk
