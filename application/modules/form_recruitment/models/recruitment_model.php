...
class recruitment_model extends CI_Model {
    ...
    {       
        ...
        //die(print_mz(get_user_satu_bu($sess_nik)));
        }
        $this->db->select(array(
                ...
                'users_recruitment'.'.approval_status_id_hrd',
                'users_recruitment'.'.user_app_lv1',
                'users_recruitment'.'.user_app_lv2',
                'users_recruitment'.'.user_app_lv3',
               'users'.'.username',
               'users'.'.nik',
            ));
            ...
            if($sess_nik == 'RECRUIT')
            {
                ...
                if($is_approver == $sess_nik || $is_admin_cabang == 1){
                    ...
                    if($sess_nik == 'P1493'){
                        ...
                        //$this->db->query('(users.nik like P)');
                    }else{
                        $this->db->where_in("users_recruitment.user_id", $user);//print_mz($user);    
                    }
                }elseif($is_admin!=1 ){
     ...
     public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        //filter out any data passed that doesnt have a matching column in the form recruitment table
        //and merge the set group data and the additional data

        if (!empty($additional_data)) $data = array_merge($additional_data, $data);


        // insert the new form_recruitment
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('users_recruitment', $data);
    }
