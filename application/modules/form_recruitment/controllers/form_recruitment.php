...
class Form_recruitment extends MX_Controller {
  ...
  var $form_name = 'recruitment';
    ...
    { 
        ...
        elseif(!is_user_app_lv1($nik,$id,'users_recruitment')&&!is_user_app_lv2($nik,$id,'users_recruitment')&&!is_user_app_lv3($nik,$id,'users_recruitment')&&!is_creator($nik,$id,'users_recruitment')&&!is_admin()&&!is_admin_bagian()&&!is_hrd_cabang($bu)&&!is_hrd_pusat()&&!is_cc_notif($nik,$bu,9)){
            return show_error('Anda tidak dapat mengakses halaman ini.');
        }else{
            $this->data['id'] = $id;
