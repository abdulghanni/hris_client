...
class form_absen extends MX_Controller {
    ...
    function send_notif_tambahan($id)
    {
        ...
        $user_id = getValue('user_id', 'users_absen', array('id'=>'where/'.$id));
        $receiver = getValue('user_nik', 'users_notif_tambahan', array('form_type_id'=>'where/2'));
        $subject_email = 'Keterangan Tidak Absen';
        $isi_email = 'HRD telah menyetujui pengajuan Keterangan Tidak Absen oleh '.get_name($user_id).', untuk melihat detail silakan <a class="klikmail" href='.$url.'>Klik Disini</a><br />';
        //Notif to karyawan
        if(!empty($receiver)){
            $data4 = array(
