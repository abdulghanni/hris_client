...
class Form_absen_model extends CI_Model
{
    ...
    public function form_absen()
    {
        ...
        else
        {
            ...
            $this->db->select(array(
                $this->tables['users_absen'].'.*',
                $this->tables['users_absen'].'.id as id',
            ...
            if($is_admin!=1){
                ...
                $this->db->or_where('users_absen.user_id', $sess_id);
                $this->db->or_where('users_absen.user_app_lv1', $sess_nik);
                $this->db->or_where('users_absen.user_app_lv2', $sess_nik);
                $this->db->or_where('users_absen.user_app_lv3', $sess_nik);
            }
    ...
    public function delete($id)
    {
        ...
        // delete organization from users_absen table
        $this->db->delete($this->tables['users_absen'], array('id' => $id));
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_absen'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_absen
        $this->db->insert($this->tables['users_absen'], $data);
    ...
    public function frm_absen($id = NULL)
    {
        ...
        $this->limit(1);
        $this->where($this->tables['users_absen'].'.id', $id);
