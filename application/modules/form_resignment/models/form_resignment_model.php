...
class Form_resignment_model extends CI_Model
{
    ...
    {
         $this->trigger_events('form_resignment_input');
    ...
    {
        $this->trigger_events('pre_delete_frm_resignment');

        $this->db->trans_begin();

        // delete organization from users_resignment table
        $this->db->delete($this->tables['users_resignment'], array('id' => $id));
        ...
        if ($this->db->trans_status() === FALSE)
        {
            ...
            $this->trigger_events(array('post_delete_frm_resignment', 'post_delete_frm_resignment_unsuccessful'));
        ...
        $this->trigger_events(array('post_delete_frm_resignment', 'post_delete_frm_resignment_successful'));
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_resignment'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_resignment
        $this->db->insert($this->tables['users_resignment'], $data);
        $id = $this->db->insert_id();

        // report success
        $this->set_message('frm_resignment_creation_successful');
    ...
    public function frm_resignment($id = NULL)
    {
        $this->trigger_events('frm_resignment');

        $this->limit(1);
        $this->where($this->tables['users_resignment'].'.id', $id);

        $this->form_resignment();
