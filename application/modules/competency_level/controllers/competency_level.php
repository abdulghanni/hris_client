...
class Competency_level extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('competency_level_model','competency_level_model');
    }

    var $title = 'Education Group';
    var $limit = 100000;
    var $controller_name = 'competency_level';
    var $model_name = 'competency_level_model';
    var $id_table = 'id';
    var $list_view = 'competency_level/index';
    ...
    function index($id=NULL)
    {
        ...
        {
            $data['url_ajax_list'] = site_url('competency_level/ajax_list');
            $data['url_ajax_add'] = site_url('competency_level/ajax_add');
    ...
    {
        $list = $this->competency_level_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $val) {
            $no++;
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->competency_level_model->count_all(),
                        "recordsFiltered" => $this->competency_level_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function ajax_add()
    {
        ...
        $data = array(
                ...
                'competency_def_id' => $this->input->post('competency_def_id'),
                'created_on' => date('Y-m-d H:i:s', now()),
                'created_by' => GetUserID()
                );
        $insert = $this->competency_level_model->save($data);
    ...
    public function ajax_delete($id)
    {
        ...
        $this->competency_level_model->update(array('id' => $id), $data);
        echo json_encode(array("status" => TRUE));
    }

    public function ajax_edit($id)
    {
        $data = $this->competency_level_model->get_by_id($id);
    ...
    public function ajax_update()
    {
        ...
        $this->competency_level_model->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    private function _validate()
    {
        ...
        if($this->input->post('competency_def_id') == '')
        {
            $data['inputerror'][] = 'level';
            $data['error_string'][] = 'level is required';
            $data['status'] = FALSE;
    ...
    function _render_page($view, $data=null, $render=false)
    {
        ...
        if ( ! $render)
        {
            ...
            if (in_array($view, array('competency_level/index')))
            {
                ...
                $this->template->add_js('competency_level.js');
