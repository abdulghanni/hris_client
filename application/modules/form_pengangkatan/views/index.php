          ...
          <div class="col-md-12">
              <div class="grid simple ">
                <div class="grid-title no-border">
                  <h4>Daftar Pengajuan <span class="semi-bold"><a href="<?php echo site_url('form_pengangkatan')?>">Pengangkatan Status Karyawan</a></span></h4>
                  <?php if(is_have_subordinate($sess_id)||is_admin()){?>
                  <div class="tools"> 
                    <a href="<?php echo site_url('form_pengangkatan/input') ?>" class="config"><button type="button" class="btn btn-primary btn-sm"><i class="icon-plus"></i>&nbsp;<?php echo lang('add_button');?></button></a>
                  </div>
                  <?php } ?>
                </div>
                  <div class="grid-body no-border">
                          <?php if($_num_rows>0){?>
                          <br/>   
                          <?php echo form_open(site_url('form_pengangkatan/keywords'))?>
                            <div class="row">
                                <div class="col-md-5">
                                    <div class="row">
                        ...
                        <?php echo form_close()?>    
                          <table class="table table-striped table-flip-scroll cf">
                              ...
                              <tbody>
                                <?php
                                  foreach($form_pengangkatan as $user):
                                  ...
                                  $approval_status_hrd = ($user->app_status_id_hrd == 1)? "<i class='icon-ok-sign' style='color:green;' title = 'Approved'></i>" : (($user->app_status_id_hrd == 2) ? "<i class='icon-remove-sign' style='color:red;'  title = 'Rejected'></i>"  : (($user->app_status_id_hrd == 3) ? "<i class='icon-exclamation-sign' style='color:orange;' title = 'Pending'></i>" : "<i class='icon-question' title = 'No Respond'></i>"));
                                    ...
                                    if(!empty($user->user_app_lv1) && $user->is_app_lv1 == 0 && $sess_nik == $user->user_app_lv1){
                                        $txt_app_lv1 = "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>
                                        ...
                                        $txt_app_lv1 = "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>$approval_status_lv1</a>";
                                      }else{
                                    ...
                                    if(!empty($user->user_app_lv2) && $user->is_app_lv2 == 0 && $sess_nik == $user->user_app_lv2){
                                        $txt_app_lv2 = "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>
                                                        <button type='button' class='btn btn-info btn-small' title='Make Approval'><i class='icon-edit'></i></button>
                                                        </a>";
                                      }elseif(!empty($user->user_app_lv2)){
                                        $txt_app_lv2 = "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>$approval_status_lv2</a>";
                                      }else{
                                    ...
                                    if(!empty($user->user_app_lv3) && $user->is_app_lv3 == 0 && $sess_nik == $user->user_app_lv3){
                                        $txt_app_lv3 = "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>
                                                        <button type='button' class='btn btn-info btn-small' title='Make Approval'><i class='icon-edit'></i></button>
                                                        </a>";
                                      }elseif(!empty($user->user_app_lv3)){
                                        $txt_app_lv3 = "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>$approval_status_lv3</a>";
                                      }else{
                                    ...
                                    if($this->approval->approver('pengangkatan') == $sess_nik &&$user->is_app_hrd == 0){
                                      ...
                                      $txt_app_hrd =  "<a href='".site_url('form_pengangkatan/detail/'.$user->id)."''>$approval_status_hrd</a>";
                                    }
                                  ?>
                                  <tr>
                                    <td><a href="<?php echo site_url('form_pengangkatan/detail/'.$user->id)?>">
                                      <?php
                                    ...
                                    <td><a href="<?php echo site_url('form_pengangkatan/detail/'.$user->id)?>"><?php echo get_name($user->user_id)?></a></td>
                                    <td><a href="<?php echo site_url('form_pengangkatan/detail/'.$user->id)?>"><?php echo get_name($user->created_by)?></a></td>
                              		<?php 
                              			$status_id = getValue('status_pengangkatan_id', 'users_pengangkatan', array('id'=>'where/'.$user->id));
                                    $status = getValue('title', 'empl_status', array('id'=>'where/'.$status_id));
                            			?>
                                    ...
                                    <td class="text-center">
                                       <a href="<?php echo site_url('form_pengangkatan/form_pengangkatan_pdf/'.$user->id)?>" target="_blank"><i class="icon-print"></i></a>
                                    </td>
                                  </tr> 
                                <?php endforeach;}?>
                              </tbody>
