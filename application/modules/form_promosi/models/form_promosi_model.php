...
class Form_promosi_model extends CI_Model {
    ...
    private function _get_datatables_query($f)
    {
        ...
        $this->db->select(array(
                'users_promosi'.'.id as id',
                'users_promosi'.'.app_status_id_lv1',
                'users_promosi'.'.app_status_id_lv2',
                'users_promosi'.'.app_status_id_lv3',
                'users_promosi'.'.app_status_id_hrd',
                'users_promosi'.'.user_app_lv1',
                'users_promosi'.'.user_app_lv2',
                'users_promosi'.'.user_app_lv3',
                'users_promosi'.'.created_by',
                $this->table.'.created_on',
                $this->table.'.old_pos',
                $this->table.'.new_pos',
                $this->table.'.date_promosi',
            ...
            $this->db->from('users_promosi');


            $this->db->join($this->join1, 'users_promosi.user_id = users.id', 'left');
            $this->db->join($this->join1.' as pengaju', 'users_promosi.created_by = pengaju.id', 'left');
            ...
            if($is_approver == $sess_nik || $is_admin_cabang == 1){
                $this->db->where_in("users_promosi.user_id", $user);//print_mz($user);
            }elseif($is_admin!=1 ){
    ...
    function detail($id){
        $this->db->select(array(
            ...
            $this->db->where('users_promosi.is_deleted', 0);
            $this->db->where('users_promosi.id', $id);
