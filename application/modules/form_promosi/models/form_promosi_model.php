...
class Form_promosi_model extends CI_Model
{
     ...
     * form_promosi
     *
     * @return object form_promosi
     * @author Deni
     **/
    public function form_promosi($id = null)
    {
        $this->trigger_events('form_promosi');
        ...
        else
        {
            ...
            $this->db->join('approval_status as status_lv4', 'users_promosi.app_status_id_lv4 = status_lv4.id', 'left');
            $this->db->join('approval_status as status_lv5', 'users_promosi.app_status_id_lv5 = status_lv5.id', 'left');
            $this->db->join('approval_status as status_hrd', 'users_promosi.app_status_id_hrd = status_hrd.id', 'left');
            $this->db->where('users_promosi.is_deleted', 0);
            if($id != null)$this->db->where('users_promosi.id', $id);
            if($is_approver == $sess_nik || $is_admin_cabang == 1){
                $this->db->where_in("users_promosi.user_id", $user);
            }elseif($is_approver !== $sess_nik && $is_admin!=1){
                //$this->db->where("(users_promosi.user_id= $sess_id $sub_id $subsub_id )",null, false);
                $this->db->where("(users_promosi.user_id = $sess_id OR  users_promosi.user_app_lv1 = '$sess_nik' OR users_promosi.user_app_lv2 = '$sess_nik' OR users_promosi.user_app_lv3 = '$sess_nik' OR users_promosi.user_app_lv4 = '$sess_nik' OR users_promosi.user_app_lv5 = '$sess_nik' OR users_promosi.created_by = '$sess_id')",null, false);
            }
            
            
            $this->db->order_by('users_promosi.id', 'desc');
        }
        ...
        $this->response = $this->db->get($this->tables['users_promosi']);
    ...
    {
         $this->trigger_events('form_promosi_input');
    ...
    {
        $this->trigger_events('pre_delete_frm_promosi');

        $this->db->trans_begin();

        // delete organization from users_promosi table
        $this->db->delete($this->tables['users_promosi'], array('id' => $id));
        ...
        if ($this->db->trans_status() === FALSE)
        {
            ...
            $this->trigger_events(array('post_delete_frm_promosi', 'post_delete_frm_promosi_unsuccessful'));
        ...
        $this->trigger_events(array('post_delete_frm_promosi', 'post_delete_frm_promosi_successful'));
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        //filter out any data passed that doesnt have a matching column in the form promosi table
        //and merge the set group data and the additional data
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_promosi'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_promosi
        $this->db->insert($this->tables['users_promosi'], $data);
        $id = $this->db->insert_id();

        // report success
        $this->set_message('frm_promosi_creation_successful');
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('users_promosi', $data);
    }
    ...
    public function frm_promosi($id = NULL)
    {
        $this->trigger_events('frm_promosi');

        $this->limit(1);
        $this->where($this->tables['users_promosi'].'.id', $id);
