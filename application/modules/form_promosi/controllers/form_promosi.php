...
class Form_promosi extends MX_Controller {
    ...
    function detail($id, $lv = null)
    {
        ...
        if (!$this->ion_auth->logged_in())
        {
            ...
            //redirect them to the login page
            redirect('auth/login', 'refresh');
        }elseif(!is_user_app_lv1($nik,$id,'users_promosi')&&!is_user_app_lv2($nik,$id,'users_promosi')&&!is_user_app_lv3($nik,$id,'users_promosi')&&!is_user_app_lv4($nik,$id,'users_promosi')&&!is_user_app_lv5($nik,$id,'users_promosi')&&!is_admin()&&!is_hrd_cabang($bu)&&!is_hrd_pusat($nik,5)&&!is_user_logged($nik,$id,'users_promosi')&&!is_creator($nik,$id,'users_promosi')&&!is_cc_notif($nik,$bu,5)){
            return show_error('Anda tidak dapat mengakses halaman ini.');
        }else{
            $this->data['id'] = $id;
            $sess_id = $this->data['sess_id'] = $this->session->userdata('user_id');
            ...
            $this->data['user_folder'] = $user_id.trim($first_name).'/promosi/';
    ...
    function upload_attachment($id)
    {
        ...
        if(!is_dir("./uploads/$user_folder/promosi/")){
        mkdir("./uploads/$user_folder/promosi/", 0777);
        }


        $path = "./uploads/$user_folder/promosi/";
