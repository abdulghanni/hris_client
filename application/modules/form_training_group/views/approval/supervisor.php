        ...
        <div class="col-md-12">
          <div class="grid simple">
            ...
            <div class="grid-body no-border">
              ...
              <?php if($form_training_group->num_rows()>0){
                ...
                <div class="col-md-12">
                  ...
                  <?php if ($user->is_app_lv1 == 1 && cek_subordinate(is_have_subordinate($session_id),'id', $user->user_pengaju_id) == TRUE) { ?>
                  <div class="row form-row">
                    <div class="col-md-6">
                      &nbsp;
                    </div>
                    <div class="col-md-6">
                      <div class='btn btn-info btn-small' class="text-center" title='Edit Approval' data-toggle="modal" data-target="#updateapprovetrainingModal"><i class='icon-edit'> Edit Approval</i></div>
                    </div>
                  </div>
                  <?php } ?>
                </div>
                    ...
                    <div class="row wf-cuti">
                      ...
                      <div class="col-md-4">
                        ...
                        <?php 
                            ...
                            <?php }elseif($user->is_app_lv1 == 1 && cek_subordinate(is_have_subordinate($session_id),'id', $user->user_pengaju_id) == FALSE){?>
                            <span class="semi-bold"><?php echo get_name($user->user_app_lv1)?></span><br/>
                            <span class="small"><?php echo dateIndo($user->date_app_lv1)?></span>
                            <?php }elseif(cek_subordinate(is_have_subordinate($session_nik),'id', $user->user_pengaju_id))
                                  {
                                    if($user->is_app_lv1 == 0){?>
                          <div class="btn btn-success btn-cons" data-toggle="modal" data-target="#doapprovetrainingModal"><i class="icon-ok"></i>Submit</div>
                          <?php }}?>
                      </div>
                       <div class="col-md-4">
...
<!-- Do approval training Modal -->
<div class="modal fade" id="doapprovetrainingModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog" id="modaldialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-no-horizontal-spacing" method="POST" action="<?php echo site_url('form_training_group/do_approve_spv/'.$this->uri->segment(3))?>">
            <div class="row form-row">
              <div class="col-md-12">
...
<div class="modal fade" id="updateapprovetrainingModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog" id="modaldialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      ...
      <p class="error_msg" id="MsgBad" style="background: #fff; display: none;"></p>
      <div class="modal-body">
        <form class="form-no-horizontal-spacing" method="POST" action="<?php echo site_url('form_training_group/update_approve_spv/'.$this->uri->segment(3))?>">
            <div class="row form-row">
              <div class="col-md-12">
                <label class="form-label text-left">Status Approval </label>
              </div>
                ...
                <div class="radio">
                  ...
                  if($approval_status->num_rows() > 0){
                    foreach($approval_status->result() as $app){
                      $checked = ($app->id <> 0 && $app->id == $approval_id) ? 'checked = "checked"' : '';
                      ?>
                  <input id="update_app_status<?php echo $app->id?>" type="radio" name="app_status_update" value="<?php echo $app->id?>" <?php echo $checked?>>
                  <label for="update_app_status<?php echo $app->id?>"><?php echo $app->title?></label>
                  <?php }}else{?>
                  <input id="update_app_status" type="radio" name="app_status" value="0">
                  <label for="update_app_status">No Data</label>
                    <?php } ?>
                </div>
              </div>
            </div>
            <div class="row form-row">
              ...
              <div class="col-md-12">
                <textarea name="note_spv_update" class="custom-txtarea-form" placeholder="Note supervisor isi disini"><?=$notes_spv?></textarea>
              </div>
            </div>
      </div>
      <div class="modal-footer">
