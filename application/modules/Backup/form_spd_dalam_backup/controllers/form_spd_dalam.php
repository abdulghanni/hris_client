...
class Form_spd_dalam extends MX_Controller {
    ...
    public function do_submit($id)
    {
        ...
        $receiver_id = $this->db->where('id', $id)->get('users_spd_dalam')->row('created_by');
    ...
    public function add()
    {
        ...
        else
        {
            ...
            $num_rows = $this->form_spd_dalam_model->form_spd_dalam()->num_rows();
            ...
            {
                $this->send_spd_mail($spd_id, $user_id);
    ...
    public function report($id)
    {
        ...
        else
        {
            ...
            $receiver_user_id = $this->db->where('id', $id)->get('users_spd_dalam')->row('user_id');
            ...
            $data_result = $this->data['task_detail'] = $this->form_spd_dalam_model->where('users_spd_dalam.id',$id)->form_spd_dalam()->result();
            $this->data['td_num_rows'] = $this->form_spd_dalam_model->where('users_spd_dalam.id',$id)->form_spd_dalam()->num_rows();

            $this->data['user_folder'] = $user_folder = $this->db->where('id', $id)->get('users_spd_dalam')->row('user_id');

            $report = $this->data['report'] = $this->form_spd_dalam_model->where('users_spd_dalam_report.user_spd_dalam_id', $id)->form_spd_dalam_report()->result();
            $n_report = $this->data['n_report'] = $this->form_spd_dalam_model->where('users_spd_dalam_report.user_spd_dalam_id', $id)->form_spd_dalam_report()->num_rows();
            //print_mz($report->result());
            if($n_report==0){
                $this->data['tujuan'] = '';
                $this->data['hasil'] = '';
    ...
    public function add_report($spd_id)
    {
        ...
        else
        {
            ...
            $user_folder = $this->db->where('id', $spd_id)->get('users_spd_dalam')->row('user_id');
            ...
            mkdir('./uploads/pdf/'.$user_folder, 0777);
                ...
                $receiver_id = $this->db->where('id', $spd_id)->get('users_spd_dalam')->row('created_by');
    ...
    function send_spd_mail($spd_id, $receiver_id)
    {
