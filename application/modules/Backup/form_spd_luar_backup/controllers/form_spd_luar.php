...
class Form_spd_luar extends MX_Controller {
    ...
    public function do_submit($id)
    {
        ...
        $receiver_id = $this->db->where('id', $id)->get('users_spd_luar')->row('created_by');
    ...
    public function report($id)
    {
        ...
        else
        {
            ...
            $receiver_user_id = $this->db->where('id', $id)->get('users_spd_luar')->row('user_id');
            ...
            $this->data['user_folder'] = $user_folder = $this->db->where('id', $id)->get('users_spd_luar')->row('user_id');
    ...
    public function add_report($spd_id)
    {
        ...
        else
        {
            ...
            $user_folder = $this->db->where('id', $spd_id)->get('users_spd_luar')->row('user_id');
            ...
            mkdir('./uploads/pdf/'.$user_folder, 0777);
                ...
                $receiver_id = $this->db->where('id', $spd_id)->get('users_spd_luar')->row('created_by');
    ...
    function send_spd_submitted_mail($spd_id, $receiver_id)
    {
        ...
        $data = array(
                    ...
                    'email_body' => get_name($sender).' telah menyetujui tugas perjalan dinas luar kota yang anda berikan, untuk melihat detail silakan <a href='.$url.'>Klik Disini</a>',
                    'is_read' => 0,
                );
        $this->db->insert('email', $data);
    }
    ...
    function send_spd_report_mail($spd_id, $receiver_id)
    {
        ...
        $data = array(
                    ...
                    'email_body' => get_name($sender).' telah membuat laporan Perjalanan Dinas Luar Kota, untuk melihat detail silakan <a href='.$url.'>Klik Disini</a>',
                    'is_read' => 0,
                );
            $this->db->insert('email', $data);
    } 
