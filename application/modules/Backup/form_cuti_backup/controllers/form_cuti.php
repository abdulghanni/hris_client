...
class Form_cuti extends MX_Controller {
    ...
    {   //print_mz($this->load->view('form_cuti/page_prints'));
    ...
    { 
        ...
        else
        {
           ...
           if ($cuti_num_rows > 0){
			...
			$url_pengganti = 'http://admin:12345678@localhost/hris_api/users/employement/EMPLID/'.$user_pengganti.'/format/json';
    ...
    public function add()
    {
        ...
        else
        {
            ...
            $num_rows = $this->form_cuti_model->form_cuti()->num_rows();
            ...
            if ($this->form_validation->run() == true && $this->form_cuti_model->create_($user_id,$additional_data))
            {
                 ...
                 $this->send_approval_request($cuti_id);
    ...
    function send_approval_request($id)
    {
        ...
        if(is_have_superior($user_id))
        {
            $data = array(
                    ...
                    'receiver_id' => $this->form_cuti_model->get_superior_id($user_id)->row('superior_id'),
                    'sent_on' => date('Y-m-d-H-i-s',strtotime('now')),
                    'subject' => 'Pengajuan Permohonan Cuti',
                    'email_body' => $username.' mengajukan permohonan cuti, untuk melihat detail silakan <a href='.$url.'spv/'.$id.'>Klik Disini</a>',
                    'is_read' => 0,
                );
        ...
        if(!empty(get_superior($current_superior)))
        {
            $data = array(
                    ...
                    'sent_on' => date('Y-m-d-H-i-s',strtotime('now')),
                    'subject' => 'Pengajuan Permohonan Cuti',
                    'email_body' => $username.' mengajukan permohonan cuti, untuk melihat detail silakan <a href='.$url.'kbg/'.$id.'>Klik Disini</a>',
                    'is_read' => 0,
                );
            $this->db->insert('email', $data);
        }
        ...
        $data = array(
                ...
                'sent_on' => date('Y-m-d-H-i-s',strtotime('now')),
                'subject' => 'Pengajuan Permohonan Cuti',
                'email_body' => $username.' mengajukan permohonan cuti, untuk melihat detail silakan <a href='.$url.'hrd/'.$id.'>Klik Disini</a>',
                'is_read' => 0,
            );
        $this->db->insert('email', $data);
    }
