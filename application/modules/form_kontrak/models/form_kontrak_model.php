...
class Form_kontrak_model extends CI_Model
{
     /**
     ...
     * form_kontrak
     *
     * @return object form_kontrak
     * @author Deni
     **/
    public function form_kontrak($id = null)
    {
        $this->trigger_events('form_kontrak');
        ...
        else
        {
            ...
            $is_approver = $this->approval->approver('kontrak');
            $is_admin = is_admin();
            ...
            $this->db->select(array(
                $this->tables['users_kontrak'].'.*',
                $this->tables['users_kontrak'].'.id as id',
            ...
            $this->db->join('users', 'users.id = users_kontrak.user_id', 'LEFT');
            $this->db->join('users as creator', 'creator.id = users_kontrak.created_by', 'LEFT');
            $this->db->join('approval_status as status_lv1', 'users_kontrak.app_status_id_lv1 = status_lv1.id', 'left');
            ...
            $this->db->order_by('users_kontrak.id', 'desc');
        }
        ...
        $this->response = $this->db->get($this->tables['users_kontrak']);
    ...
    {
         $this->trigger_events('form_kontrak_input');
    ...
    {
        $this->trigger_events('pre_delete_frm_kontrak');

        $this->db->trans_begin();

        // delete organization from users_kontrak table
        $this->db->delete($this->tables['users_kontrak'], array('id' => $id));
        ...
        if ($this->db->trans_status() === FALSE)
        {
            ...
            $this->trigger_events(array('post_delete_frm_kontrak', 'post_delete_frm_kontrak_unsuccessful'));
        ...
        $this->trigger_events(array('post_delete_frm_kontrak', 'post_delete_frm_kontrak_successful'));
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        //filter out any data passed that doesnt have a matching column in the form kontrak table
        //and merge the set group data and the additional data
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_kontrak'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_kontrak
        $this->db->insert($this->tables['users_kontrak'], $data);
        $id = $this->db->insert_id();

        // report success
        $this->set_message('frm_kontrak_creation_successful');
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('users_kontrak', $data);
    }
    ...
    public function frm_kontrak($id = NULL)
    {
        $this->trigger_events('frm_kontrak');

        $this->limit(1);
        $this->where($this->tables['users_kontrak'].'.id', $id);
