...
class Form_kontrak_model extends CI_Model {
    ...
    private function _get_datatables_query($f)
    {
        ...
        $this->db->select(array(
                'users_kontrak'.'.id as id',
                'users_kontrak'.'.app_status_id_lv1',
                'users_kontrak'.'.app_status_id_lv2',
                'users_kontrak'.'.app_status_id_lv3',
                'users_kontrak'.'.app_status_id_hrd',
                'users_kontrak'.'.user_app_lv1',
            ...
            $this->db->from('users_kontrak');


            $this->db->join($this->join1, 'users_kontrak.user_id = users.id', 'left');
            $this->db->join($this->join1.' as pengaju', 'users_kontrak.created_by = pengaju.id', 'left');
            $this->db->join($this->join2, 'users_kontrak.lama_kontrak = lama_kontrak.id', 'left');
            ...
            if($is_admin!=1 && $is_hrd_pusat != 1):
                if($is_approver == $sess_nik || $is_admin_cabang == 1){
                    ...
                    if($sess_nik == 'P1493' || $sess_nik == 'P0093'){
                     ...
                     $this->db->where("(users_kontrak.user_id = '$sess_id'
                                   OR users_kontrak.user_app_lv1 = '$sess_nik'  OR users_kontrak.user_app_lv2 = '$sess_nik'  OR users_kontrak.user_app_lv3 = '$sess_nik'  OR users_kontrak.created_by = '$sess_id'
                    )",null, false);
                }
            endif;
    ...
    function detail($id){
        $this->db->select(array(
            ...
            $this->db->where('users_kontrak.is_deleted', 0);
            $this->db->where('users_kontrak.id', $id);
