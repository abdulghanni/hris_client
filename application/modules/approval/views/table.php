...
<table class="table table-bordered">
    ...
    $filter = array('bu'=>'where/'.$bu, 'form_type_id'=>'where/'.$f->id);
        $num_rows = getValue('user_nik', 'users_approval',$filter);//lastq();
        $nik = (!empty($num_rows))?getValue('user_nik', 'users_approval', $filter):"-";
        $nama = (!empty($num_rows))?get_name($nik):"HRD Belum Dipilih";
    ?>
        <tr>
