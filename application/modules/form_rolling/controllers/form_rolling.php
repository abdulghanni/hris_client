...
class Form_rolling extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->database();
        $this->load->model('form_rolling/form_rolling_model','main');
    ...
    {
        $this->data['title'] = ucfirst('Mutasi');
    ...
    function detail($id, $lv = null)
    {
        ...
        if (!$this->ion_auth->logged_in())
        {
            ...
            //redirect them to the login page
            redirect('auth/login', 'refresh');
        }elseif(!is_user_app_lv1($nik,$id,'users_rolling')&&!is_user_app_lv2($nik,$id,'users_rolling')&&!is_user_app_lv3($nik,$id,'users_rolling')&&!is_user_app_lv4($nik,$id,'users_rolling')&&!is_user_app_lv5($nik,$id,'users_rolling')&&!is_admin()&&!is_hrd_cabang($bu)&&!is_hrd_pusat($nik,7)&&!is_user_logged($nik,$id,'users_rolling')&&!is_creator($nik,$id,'users_rolling')&&!is_cc_notif($nik,$bu,7)){
            return show_error('Anda tidak dapat mengakses halaman ini.');
        }else{
           $this->data['id'] = $id;
            $sess_id = $this->data['sess_id'] = $this->session->userdata('user_id');
            ...
            $this->data['user_folder'] = $user_id.trim($first_name).'/mutasi/';
    ...
    function upload_attachment($id)
    {
        ...
        if(!is_dir("./uploads/$user_folder/mutasi/")){
        mkdir("./uploads/$user_folder/mutasi/", 0777);
        }


        $path = "./uploads/$user_folder/mutasi/";
    ...
    function send_notif($id, $type){
        ...
        if($type !== 'hrd' && $approval_status == 1){
           ...
           if(!empty($user_app)){
            ...
            switch($type){
            case 'lv1':
                $app_status = getValue('app_status_id_lv1', 'users_rolling', array('id'=>'where/'.$id));
                if($app_status == 2)$this->db->where('id', $id)->update('users_rolling', array('is_deleted'=>1));
                //$this->approval->not_approve('spd_dalam', $id, )
            break;

            case 'lv2':
                $app_status = getValue('app_status_id_lv2', 'users_rolling', array('id'=>'where/'.$id));
                if($app_status == 2)$this->db->where('id', $id)->update('users_rolling', array('is_deleted'=>1));
            ...
            case 'lv3':
                $app_status = getValue('app_status_id_lv3', 'users_rolling', array('id'=>'where/'.$id));
                if($app_status == 2)$this->db->where('id', $id)->update('users_rolling', array('is_deleted'=>1));
                for($i=1;$i<3;$i++):
            ...
            case 'lv4':
                $app_status = getValue('app_status_id_lv4', 'users_rolling', array('id'=>'where/'.$id));
                if($app_status == 2)$this->db->where('id', $id)->update('users_rolling', array('is_deleted'=>1));
                for($i=1;$i<4;$i++):
                    $receiver = getValue('user_app_lv'.$i, 'users_'.$form, array('id'=>'where/'.$id));
                    if(!empty($receiver)):
            ...
            case 'lv5':
                $app_status = getValue('app_status_id_lv5', 'users_rolling', array('id'=>'where/'.$id));
                if($app_status == 2)$this->db->where('id', $id)->update('users_rolling', array('is_deleted'=>1));
                for($i=1;$i<5;$i++):
                    $receiver = getValue('user_app_lv'.$i, 'users_'.$form, array('id'=>'where/'.$id));
                    if(!empty($receiver)):
    ...
    function form_rolling_pdf($id)
    {
        ...
        $user_id = getValue('user_id', 'users_rolling', array('id'=>'where/'.$id));
        $this->data['user_nik'] = get_nik($user_id);
        $form_rolling = $this->data['form_rolling'] = $this->main->detail($id)->result();
        $this->data['_num_rows'] = $this->main->detail($id)->num_rows();
    ...
    function pdf_blank($id=1)
    {
        ...
        $this->mpdf->AddPage('P', // L - landscape, P - portrait
            ...
            10); // margin footer
        $this->mpdf->WriteHTML($html);
        $this->mpdf->Output('form_template-mutasi.pdf', 'I');
        }
    }
