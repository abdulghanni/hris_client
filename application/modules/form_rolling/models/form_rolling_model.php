...
class Form_rolling_model extends CI_Model
{
     ...
     * form_rolling
     *
     * @return object form_rolling
     * @author Deni
     **/
    public function form_rolling($id = null)
    {
        $this->trigger_events('form_rolling');
        ...
        else
        {
            ...
            $is_approver = $this->approval->approver('rolling', $sess_nik);
            ...
            $this->db->select(array(
                $this->tables['users_rolling'].'.*',
                $this->tables['users_rolling'].'.id as id',
            ...
            $this->db->join('users', 'users.id = users_rolling.user_id', 'LEFT');
            $this->db->join('users as creator', 'creator.id = users_rolling.created_by', 'LEFT');
            $this->db->join('approval_status as status_lv1', 'users_rolling.app_status_id_lv1 = status_lv1.id', 'left');
            $this->db->join('approval_status as status_lv2', 'users_rolling.app_status_id_lv2 = status_lv2.id', 'left');
            $this->db->join('approval_status as status_lv3', 'users_rolling.app_status_id_lv3 = status_lv3.id', 'left');
            $this->db->join('approval_status as status_lv4', 'users_rolling.app_status_id_lv4 = status_lv4.id', 'left');
            $this->db->join('approval_status as status_lv5', 'users_rolling.app_status_id_lv5 = status_lv5.id', 'left');
            $this->db->join('approval_status as status_hrd', 'users_rolling.app_status_id_hrd = status_hrd.id', 'left');
            $this->db->where('users_rolling.is_deleted', 0);
            if($id != null)$this->db->where('users_rolling.id', $id);
            if($is_approver == $sess_nik || $is_admin_cabang == 1){
                $this->db->where_in("users_rolling.user_id", $user);
            ...
            $this->db->order_by('users_rolling.id', 'desc');
        }
        ...
        $this->response = $this->db->get($this->tables['users_rolling']);
    ...
    {
         $this->trigger_events('form_rolling_input');
    ...
    {
        $this->trigger_events('pre_delete_frm_rolling');

        $this->db->trans_begin();

        // delete organization from users_rolling table
        $this->db->delete($this->tables['users_rolling'], array('id' => $id));
        ...
        if ($this->db->trans_status() === FALSE)
        {
            ...
            $this->trigger_events(array('post_delete_frm_rolling', 'post_delete_frm_rolling_unsuccessful'));
        ...
        $this->trigger_events(array('post_delete_frm_rolling', 'post_delete_frm_rolling_successful'));
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_rolling'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_rolling
        $this->db->insert($this->tables['users_rolling'], $data);
        $id = $this->db->insert_id();

        // report success
        $this->set_message('frm_rolling_creation_successful');
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('users_rolling', $data);
    }
    ...
    public function frm_rolling($id = NULL)
    {
        $this->trigger_events('frm_rolling');

        $this->limit(1);
        $this->where($this->tables['users_rolling'].'.id', $id);
