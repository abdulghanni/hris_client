...
class Form_rolling_model extends CI_Model {
    ...
    var $table = 'users_rolling';
    var $join1  = 'users';
    var $column = array('users_rolling.id', 'nik_karyawan','karyawan', 'nik_pengaju', 'pengaju', 'old_pos', 'new_pos','date_rolling', 'created_on'); //set column field database for order and search
    ...
    private function _get_datatables_query($f)
    {
        ...
        $this->db->select(array(
                'users_rolling'.'.id as id',
                'users_rolling'.'.app_status_id_lv1',
                'users_rolling'.'.app_status_id_lv2',
                'users_rolling'.'.app_status_id_lv3',
                'users_rolling'.'.app_status_id_hrd',
                'users_rolling'.'.user_app_lv1',
                'users_rolling'.'.user_app_lv2',
                'users_rolling'.'.user_app_lv3',
                'users_rolling'.'.created_by',
            ...
            $this->db->from('users_rolling');


            $this->db->join($this->join1, 'users_rolling.user_id = users.id', 'left');
            $this->db->join($this->join1.' as pengaju', 'users_rolling.created_by = pengaju.id', 'left');
    ...
    function detail($id){
        $this->db->select(array(
            ...
            $this->db->where('users_rolling.is_deleted', 0);
            $this->db->where('users_rolling.id', $id);
