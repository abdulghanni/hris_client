...
<?php foreach($form_kenaikan_gaji as $row):
...
<div class="style4">
  ...
  Perihal : Pengajuan Kenaikan Gaji Karyawan <?= get_nik($row->user_id).' '.get_name($row->user_id)?><br/><br/>
  Kepada Yth.,<br/>
  Departemen HRD<br/>
  Di Tempat<br/>

  <p>Dengan hormat,</p>
  <p>Sesuai evaluasi kinerja karyawan terlampir, bersama ini kami sampaikan permohonan proses kenaikan gaji karyawan atas nama <?= get_name($row->user_id).' ('.get_nik($row->user_id).')' ?> dengan data sebagai berikut:</p>
</div>
  ...
  <table width="800" height="128" border-style:solid border="1" class="tg">
    <thead>
      ...
      <tr>
        <td><span class="style3">Tanggal Kenaikan Gaji </span></td>
        <td><div align="center"><?php echo dateIndo($row->date_rolling)?></div></td>
        <td><span class="style3"></span></td>
      </tr>
      <tr>
        <td height="100"><span class="style3">Alasan Kenaikan Gaji</span></td>
...
<?php if($komponen->num_rows() > 0) { ?>
  <h5>Komponen gaji yang diajukan</h5>
  <table width="800" height="128" border-style:solid border="1" class="tg">
    <thead>
      <tr>
        <th width="50%">Baru</th>
    ...
    <tbody>
      <?php foreach ($komponen->result_array() as $key => $value) { ?>
        <tr>
          <td><?php echo $value['new_komponen']?></td>
          <td><?php echo $value['old_komponen']?></td>
        </tr>
      <?php } ?>
    </tbody>
  </table>
