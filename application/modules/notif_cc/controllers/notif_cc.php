...
class notif_cc extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->database();
        //$this->load->model('notif_cc_model');
        $this->lang->load('auth');
    ...
    { 
        ...
        else
        {
            ...
            $this->data['form_type'] = $this->db->get('form_id');
            $this->get_bu();
            $this->_render_page('notif_cc/index', $this->data);
        }
    }
    ...
    function get_table(){
        ...
        $id_list = array('1','2','9', '10', '13', '14');
            ...
            $this->data['form'] = $this->db->get('form_id')->result();
        $this->load->view('notif_cc/table', $this->data);
    }

    function get_table_by_name(){
        // $this->load->model('notif_cc_model', 'app');
        $name = $this->input->post('name');
        $this->data['bu'] = $bu = substr($this->input->post('bu'),0,2);//print_mz($this->data['bu']);
        $this->data['form'] = $this->app->get_table($bu, $name)->result();
        $this->load->view('notif_cc/table_name', $this->data);
    }

    function get_modal($bu, $form_id){
        $data = getValue('user_nik', 'users_notif_cc', array('bu'=>'where/'.$bu, 'form_type_id'=>'where/'.$form_id));
        echo $data;
    }
    ...
    function update()
    {
        ...
        elseif (!$this->ion_auth->is_admin()) //remove this elseif if you want to enable this for non-admins
        {
            ...
            $num = getAll('users_notif_cc', array('form_type_id'=>'where/'.$form, 'bu'=>'where/'.$bu))->num_rows();
            if($num>0)$this->db->where('bu',$bu)->where('form_type_id', $form)->update('users_notif_cc', $data);
            else $this->db->insert('users_notif_cc', $data);
    ...
    function _render_page($view, $data=null, $render=false)
    {
        ...
        if ( ! $render)
        {
            $this->load->library('template');
                ...
                if(in_array($view, array('notif_cc/index')))
                {
                    ...
                    $this->template->add_js('notif_cc.js');
