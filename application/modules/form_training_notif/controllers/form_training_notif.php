...
class form_training_notif extends MX_Controller {

    public $data;
    var $form_name = 'training_notif';
    function __construct()
    {
        ...
        $this->load->database();
        $this->load->model('form_training_notif/form_training_notif_model','main');
    ...
    function index($ftitle = "fn:",$sort_by = "id", $sort_order = "asc", $offset = 0)
    {
        ...
        else
        {
            ...
            $this->_render_page('form_training_notif/index', $this->data);
        }
    }
    ...
    public function ajax_list($f)
    {
        ...
        foreach ($list as $r) {
           ...
           $delete = (($r->approval_status_id_lv1 == 0 && $r->created_by == sessId()) || is_admin()) ? '<button onclick="showModal('.$r->id.')" class="btn btn-sm btn-danger" type="button" title="Batalkan Pengajuan"><i class="icon-remove"></i></button>' : '';
            ...
            $row[] = "<a href=$detail>".dateIndo($r->tanggal_mulai).' s/d '.dateIndo($r->tanggal_akhir).'</a>';
            $row[] = "<a href=$detail>".$r->training_name.'</a>';
            $row[] = $r->nik;
            $row[] = $r->username;
            $row[] = $status1;
    ...
    function detail($id, $lv = null)
    {
        ...
        {
            $this->data['id'] = $id;
            $user_id= getValue('user_pengaju_id', 'users_training_notif', array('id'=>'where/'.$id));
    ...
    function input()
    {
       ...
       // $form_training_notif = $this->data['training'] = $this->main->form_training_notif($sess_id);
    ...
    function add()
    {
        ...
        else
        {
            ...
            //echo json_encode(array('st'=>0, 'errors'=>validation_errors('<div class="alert alert-danger" role="alert">', '</div>')));
                redirect('form_training_notif/input','refresh');
            }
            else
            {
                ...
                //$peserta_id = explode(',',$peserta_id);
                    ...
                    if ($this->form_validation->run() == true && $this->main->create_($user_id, $data))
                    {
                        ...
                        if(!empty($user_app_lv1)){
                            $this->approval->request('lv1', 'training_notif', $training_id, $user_id, $this->detail_email($training_id));
                        if(!empty(getEmail($user_app_lv1)))
                            $this->send_email(getEmail($user_app_lv1), $subject_email, $isi_email);
                        }else{
                            $this->approval->request('hrd', 'training_notif', $training_id, $user_id, $this->detail_email($training_id));
                        if(!empty(getEmail($this->approval->approver('training'))))
                            $this->send_email(getEmail($this->approval->approver('training')), $subject_email, $isi_email);
                        }   
                        //$this->send_peserta_mail($training_id, $user_id, $peserta_id);
                        redirect('form_training_notif', 'refresh'); 
                    }
            }
    ...
    function send_notif($id, $type){
        $user_id = sessNik();
        $approval_status = getValue('approval_status_id_'.$type, 'users_training_notif', array('id'=>'where/'.$id));
        ...
        $is_app = getValue('is_app_'.$type, 'users_training_notif', array('id'=>'where/'.$id));
        if($is_app==0){
        ...
        {
            $pengaju_id = getValue('user_pengaju_id', 'users_training_notif', array('id'=>'where/'.$id));
            $lv = substr($type, -1)+1;
            $lv_app = 'lv'.$lv;
            $user_app = ($lv<4) ? getValue('user_app_'.$lv_app, 'users_training_notif', array('id'=>'where/'.$id)):0;
            if(!empty($user_app)){
                $this->approval->request($lv_app, 'training_notif', $id, $pengaju_id, $this->detail_email($id));
                if(!empty(getEmail($user_app)))$this->send_email(getEmail($user_app), $subject_email_request, $isi_email_request);
            }else{
                $this->approval->request('hrd', 'training_notif', $id, $pengaju_id, $this->detail_email($id));
            ...
            $email_body = "Status pengajuan permohonan Training yang diajukan oleh ".get_name($user_training_id).' '.$approval_status_mail. ' oleh '.get_name($user_id).' untuk detail silakan <a href='.base_url().'form_training_notif/detail/'.$id.'>Klik Disini</a> atau <a href="http://123.231.241.12/hris_client/form_training_notif/detail/'.$id.'">Klik Disini</a> jika anda akan mengakses diluar jaringan perusahaan. <br />';
            switch($type){
                case 'lv1':
                    $app_status = getValue('approval_status_id_lv1', 'users_training_notif', array('id'=>'where/'.$id));
                    if($app_status == 2)$this->db->where('id', $id)->update('users_training_notif', array('is_deleted'=>1));
                    //$this->approval->not_approve('training_notif', $id, )
                break;

                case 'lv2':
                    $app_status = getValue('approval_status_id_lv2', 'users_training_notif', array('id'=>'where/'.$id));
                    if($app_status == 2)$this->db->where('id', $id)->update('users_training_notif', array('is_deleted'=>1));
                    $receiver_id = getValue('user_app_lv1', 'users_training_notif', array('id'=>'where/'.$id));
                    $this->approval->not_approve('training_notif', $id, $receiver_id, $approval_status ,$this->detail_email($id));
                ...
                case 'lv3':
                    $app_status = getValue('approval_status_id_lv3', 'users_training_notif', array('id'=>'where/'.$id));
                    if($app_status == 2)$this->db->where('id', $id)->update('users_training_notif', array('is_deleted'=>1));
                    $receiver_lv2 = getValue('user_app_lv2', 'users_training_notif', array('id'=>'where/'.$id));
                    $this->approval->not_approve('training_notif', $id, $receiver_lv2, $approval_status ,$this->detail_email($id));
                    ...
                    $receiver_lv1 = getValue('user_app_lv1', 'users_training_notif', array('id'=>'where/'.$id));
                    $this->approval->not_approve('training_notif', $id, $receiver_lv1, $approval_status ,$this->detail_email($id));
                ...
                case 'hrd':
                    $receiver_lv3 = getValue('user_app_lv3', 'users_training_notif', array('id'=>'where/'.$id));
                    if(!empty($receiver_lv3)):
                        $this->approval->not_approve('training_notif', $id, $receiver_lv3, $approval_status ,$this->detail_email($id));
        ...
        if($type == 'hrd' && $approval_status == 1){
            $this->send_notif_tambahan($id, 'training_notif');
        }
    }
    ...
    function do_approve_hrd($id)
    {
        ...
        $is_app = getValue('is_app_hrd', 'users_training_notif', array('id'=>'where/'.$id));
    ...
    function notif_legal($id)
    {
        ...
        $user_id = getValue('user_pengaju_id', 'users_training_notif', array('id'=>'where/'.$id));
        $user_hrd = getValue('user_app_hrd', 'users_training_notif', array('id'=>'where/'.$id));
        $msg = 'Dear Admin Legal,<br/><p>'.get_name($user_hrd).' telah menyetujui permintaan pelatihan yang diajukan oleh '.get_name($user_id).' ,untuk melihat detail silakan <a class="klikemail" href="'.base_url("form_training_notif/detail/$id").'">Klik Disini</a> atau <a href="http://123.231.241.12/hris_client/form_training_notif/detail/'.$id.'">Klik Disini</a> jika anda akan mengakses diluar jaringan perusahaan. </p>';
    ...
    {
        $url = base_url().'form_training_notif/detail';
    ...
    {
        $url = base_url().'form_training_notif/detail/'.$id;
...
function get_penerima_tugas()
    ...
    function form_training_notif_pdf($id)
    {
        $this->data['title'] = 'Detail Training';
        if (!$this->ion_auth->logged_in())
        {
        ...
        else
        {
            ...
            $form_training_notif = $this->data['form_training_notif'] = $this->main->detail($id)->result();
            $this->data['_num_rows'] = $this->main->detail($id)->num_rows();
    ...
    function _render_page($view, $data=null, $render=false)
    {
        ...
        if ( ! $render)
        {
            $this->load->library('template');
                ...
                if(in_array($view, array('form_training_notif/index')))
                {
                ...
                elseif(in_array($view, array('form_training_notif/input')))
                {
                    ...
                    $this->template->add_js('form_training_notif_input.js');
                ...
                elseif(in_array($view, array('form_training_notif/detail')))
                {
                    ...
                    $this->template->add_js('emp_dropdown.js');
                    $this->template->add_js('form_training_notif.js');
