...
class form_spd_dalam_group_model extends CI_Model
{
     /**
    ...
    /**
     * form_spd_dalam_group
     *
     * @return object form_spd_dalam_group
     * @author Deni
     **/
    public function form_spd_dalam_group($id = null)
    {
        ...
        $this->response = $this->db->get($this->tables['users_spd_dalam_group']);

        return $this;
    }

    
    public function form_spd_dalam_report_group($id = null, $user_id = null)
    {
        $this->trigger_events('form_spd_dalam_report_group');
        ...
        else
        {
            ...
            $sess_nik = (!empty(get_nik($this->session->userdata('user_id'))))?get_nik($this->session->userdata('user_id')):$this->session->userdata('user_id');
            $this->db->select(array(
                $this->tables['users_spd_dalam_report_group'].'.*',
                $this->tables['users_spd_dalam_report_group'].'.id as id',
                $this->tables['users_spd_dalam_group'].'.id as spd_id',

            ));

            $this->db->join('users_spd_dalam_report_group', 'users_spd_dalam_group.id = users_spd_dalam_report_group.user_spd_dalam_group_id', 'left');
            if($id != null && $user_id != null){
                //$this->db->where('users_spd_dalam_report_group.user_spd_dalam_group_id', $id);
                $this->db->where('users_spd_dalam_report_group.created_by', $user_id);
            }
            $this->db->where('users_spd_dalam_group.is_deleted', 0);
        }
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_spd_dalam_group'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_spd_dalam_group
        $this->db->insert($this->tables['users_spd_dalam_group'], $data);
        $id = $this->db->insert_id();

        // report success
        $this->set_message('frm_spd_dalam_creation_successful');
    ...
    public function create_report($spd_id = FALSE, $additional_data = array())
    {
        ...
        $data = array('user_spd_dalam_group_id'=>$spd_id);

        //filter out any data passed that doesnt have a matching column in the form cuti table
        //and merge the set group data and the additional data
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_spd_dalam_report_group'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_spd_dalam_group
        $this->db->insert($this->tables['users_spd_dalam_report_group'], $data);
        $id = $this->db->insert_id();

        // report success
        $this->set_message('frm_spd_dalam_report_creation_successful');
        // return the brand new id
        return $id;
    }
    ...
    public function update_report($id, array $data)
    {
        ...
        $data = $this->_filter_data($this->tables['users_spd_dalam_report_group'], $data);

        $this->trigger_events('extra_where');
        $this->db->update($this->tables['users_spd_dalam_report_group'], $data, array('id' => $id));

        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();

            $this->trigger_events(array('post_update_form_spd_dalam_report_group', 'post_update_form_spd_dalam_report_group_unsuccessful'));
    ...
    public function update($id, array $data)
    {
        ...
        $form_spd_dalam_group = $this->form_spd_dalam_group($id)->row();

        $this->db->trans_begin();

        // Filter the data passed
        $data = $this->_filter_data($this->tables['users_spd_dalam_group'], $data);

        $this->trigger_events('extra_where');
        $this->db->update($this->tables['users_spd_dalam_group'], $data, array('id' => $id));

        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();

            $this->trigger_events(array('post_update_form_spd_dalam_group', 'post_update_form_spd_dalam_group_unsuccessful'));
            $this->set_error('update_unsuccessful');
            return FALSE;
        }
        ...
        $this->trigger_events(array('post_update_form_spd_dalam_group', 'post_update_frm_cuti_unsuccessful'));
        $this->set_message('update_successful');
        return TRUE;
    }
