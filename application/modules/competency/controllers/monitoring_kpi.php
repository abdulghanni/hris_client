...
class monitoring_kpi extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('monitoring_kpi_model','main');
    }

    var $title = 'Penilaian KPI';
    var $limit = 100000;
    var $controller = 'competency/monitoring_kpi';
    var $table = 'competency_form_kpi';
    var $model_name = 'monitoring_kpi_model';
    var $id_table = 'id';
    var $list_view = 'monitoring_kpi/index';
    ...
    function index($id=NULL)
    {
        ...
        else
        {
            ...
            $data['url_ajax_list'] = site_url('monitoring_kpi/ajax_list');
            $data['url_ajax_add'] = site_url('monitoring_kpi/ajax_add');
            $data['url_ajax_edit'] = site_url('monitoring_kpi/ajax_edit');
            $data['url_ajax_delete'] = site_url('monitoring_kpi/ajax_delete');
            $data['url_ajax_update'] = site_url('monitoring_kpi/ajax_update');
            $data['org'] = $this->competency->get_organization();

            $this->_render_page('monitoring_kpi/index',$data);
        }
    }
    ...
    function input($org_id = null, $comp_session_id = null){
        ...
        $data['url_ajax_add'] = site_url('competency/monitoring_kpi/ajax_add');
        $data['url_ajax_update'] = site_url('competency/monitoring_kpi/ajax_update');
        $data['url_ajax_edit'] = site_url('competency/monitoring_kpi/ajax_edit');
        $data['url_ajax_delete'] = site_url('competency/monitoring_kpi/ajax_delete');
        $this->_render_page('monitoring_kpi/input', $data);
    }
    ...
    function input_detail($org_id, $comp_session_id, $id)
    {
        ...
        if($mapping_kpi_detail->num_rows() > 0)
        {
            ...
            $kpi_detail = getAll('competency_monitoring_kpi_detail',array('organization_id'=>'where/'.$org_id,'comp_session_id'=>'where/'.$comp_session_id,'competency_mapping_kpi_detail_id'=>'where/'.$id,'is_deleted'=>'where/0'));
        ...
        $this->_render_page('monitoring_kpi/input_detail', $data);
    }
    ...
    function edit_detail($org_id, $comp_session_id, $id,$kpi_detail_id)
    {
        ...
        if($mapping_kpi_detail->num_rows() > 0)
        {
            $data['mapping_kpi'] = $mapping_kpi = $mapping_kpi_detail->row_array();
            $data['users'] = $qusers = $this->competency->get_emp_by_pos($mapping_kpi['position_group_id']);
            $kpi_detail = getAll('competency_monitoring_kpi_detail',array('organization_id'=>'where/'.$org_id,'comp_session_id'=>'where/'.$comp_session_id,'id'=>'where/'.$kpi_detail_id,'is_deleted'=>'where/0'));
            if($kpi_detail->num_rows() > 0)
            {
                $data['kpi_detail'] = $kpi_detail->row_array();
            }else{
        ...
        $this->_render_page('monitoring_kpi/edit_detail', $data);
    }
    ...
    public function ajax_delete($id)
    {
        ...
        $org_id = GetValue('organization_id','competency_monitoring_kpi_detail', array('id'=>'where/'.$id));
        $get_detail = GetJoin('competency_monitoring_kpi_detail as a','competency_monitoring as b','a.competency_monitoring_id = b.id','LEFT','a.id as id, a.organization_id as organization_id, a.position_group_id as position_group_id, a.area_kinerja_utama as area_kinerja_utama, a.kpi as kpi, a.bobot_kpi as bobot_kpi, a.target_kpi as target_kpi, a.sumber_info as sumber_info, a.competency_monitoring_id as competency_monitoring_id,a.is_deleted as is_deleted,b.title as competency_monitoring',array('a.organization_id'=>'where/'.$org_id,'a.is_deleted'=>'where/0','a.position_group_id'=>'order/asc'));
        $html_detail = "";
        //$html_detail .= '<tr>';
        foreach ($get_detail->result_array() as $key => $value) { 
    ...
    function add(){
        ...
        // INSERT TO COMPETENCY_monitoring_kpi
        if($num_rows == 0){
            $data = array(
                'organization_id' => $this->input->post('org_id'),
                'comp_session_id' => $this->input->post('comp_session_id'),
        ...
        // INSERT TO COMPETENCY_monitoring_kpi_DETAIL
        ...
        // INSERT TO COMPETENCY_monitoring_kpi_APPROVER
    ...
    function get_organization(){
        $data['org'] = $this->competency->get_organization();
        $this->load->view('monitoring_kpi/org',$data);
     }

     function get_periode(){
        $data['periode'] = $this->main->get_periode();
        $this->load->view('monitoring_kpi/periode',$data);
     }
    ...
    function get_mapping_from_org($org_id,$comp_session_id){
        ...
        $data['ci'] = $this;
        $this->load->view('monitoring_kpi/result', $data);
    }
    ...
    function get_kpi($org_id,$comp_session_id){
        ...
        $data['approver'] = GetAll('competency_monitoring_kpi_approver', array('organization_id'=>'where/'.$org_id, 'comp_session_id'=>'where/'.$comp_session_id));
        $data['data'] = $this->main->standar($org_id,$comp_session_id);
        $data['org_id'] = $org_id;
        $data['comp_session_id'] = $comp_session_id;
        ...
        $data['ci'] = $this;
        $this->load->view('monitoring_kpi/result_monitoring', $data);
    }
    ...
    function _render_page($view, $data=null, $render=false)
    {
        ...
        if ( ! $render)
        {
            ...
            if (in_array($view, array('monitoring_kpi/index')))
            {
                ...
                $this->template->add_js('competency/monitoring_kpi.js');
                    
            }elseif (in_array($view, array('monitoring_kpi/input','monitoring_kpi/input_detail','monitoring_kpi/edit_detail')))
            {
                $this->template->set_layout('default');
                $this->template->add_js('jquery-ui-1.10.1.custom.min.js');
                $this->template->add_js('jquery.sidr.min.js');
                ...
                $this->template->add_js('competency/monitoring_kpi_input.js');
